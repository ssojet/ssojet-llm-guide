...
List prerequisites tailored to $Go, including:  
- An existing $Go application with a login page.  
- Basic knowledge of $Go and its common tools/frameworks (e.g., ${FRAMEWORK_TOOLS}).  
...
- Required libraries or packages specific to $Go for OIDC (e.g., "oidc-client-ts" for JavaScript, "passport-openidconnect" for Node.js, "goth" for Golang).  
...
3. Create a new application (e.g., "My$GoApp", type **Regular Web App**).  
4. Configure the callback URI (e.g., '$/auth/callback' specific to $Go, like 'http://localhost:3000/auth/callback' for Node.js).  
...
#### Step 3.2: Modify the Existing $Go Project  

##### Substep 3.2.1: Install Dependencies  
- Specify the OIDC library installation command for $Go (e.g., 'npm install' for Node.js, 'go get' for Golang, 'dotnet add package' for .NET).  
...
- Tailor to $Go conventions (e.g., **Passport** for Node.js, **OpenIdConnect middleware** for .NET, **Goth** for Golang).  
...
- Use $Go-specific templating or UI logic (e.g., **EJS** for Node.js, **Razor** for .NET, **html/template** for Golang).  
...
- Provide code snippets specific to $Go (e.g., **Express routes** for Node.js, **Razor Page handlers** for .NET, **Mux handlers** for Golang).  
...
List considerations tailored to $Go:  
...
- Link to relevant $Go-specific OIDC library docs (e.g., **Passport for Node.js**, **Goth for Golang**).  

---

## Customization Instructions  
- Replace '$Go' with the target language (e.g., **Node.js**, **ASP.NET Core**, **Golang**).  
