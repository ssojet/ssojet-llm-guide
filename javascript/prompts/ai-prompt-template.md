# AI Prompt Template for SSOJet JavaScript Integration Guide
...
List prerequisites tailored to $JavaScript, including:  
- An existing $JavaScript application with a login page.  
- Basic knowledge of $JavaScript and its common tools/frameworks (e.g., ${FRAMEWORK_TOOLS}).  
...
- Required libraries or packages specific to $JavaScript for OIDC (e.g., "oidc-client-ts" for JavaScript, "passport-openidconnect" for Node.js, "goth" for Golang).
...
3. Create a new application (e.g., "My$JavaScriptApp", type **Regular Web App**).  
4. Configure the callback URI (e.g., '$/auth/callback' specific to $JavaScript, like 'http://localhost:3000/auth/callback' for Node.js).  
...
#### Step 3.2: Modify the Existing $JavaScript Project  

##### Substep 3.2.1: Install Dependencies  
- Specify the OIDC library installation command for $JavaScript (e.g., 'npm install' for Node.js, 'go get' for Golang, 'dotnet add package' for .NET).  
...
- Tailor to $JavaScript conventions (e.g., **Passport** for Node.js, **OpenIdConnect middleware** for .NET, **Goth** for Golang).  
...
- Use $JavaScript-specific templating or UI logic (e.g., **JavaScript components** for Next.js, **EJS** for Node.js, **Razor** for .NET, **html/template** for Golang).  
...
List considerations tailored to $JavaScript:  
...
- Link to relevant $JavaScript-specific OIDC library docs (e.g., **Passport for Node.js**, **Goth for Golang**).

---

## Customization Instructions  
- Replace '$JavaScript' with the target language (e.g., **Node.js**, **ASP.NET Core**, **Golang**).  
