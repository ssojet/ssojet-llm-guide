...
List prerequisites tailored to $iOS/Swift, including:  
- An existing $iOS/Swift application with a login page.  
- Basic knowledge of $iOS/Swift and its common tools/frameworks (e.g., ${FRAMEWORK_TOOLS}).  
...
- Required libraries or packages specific to $iOS/Swift for OIDC (e.g., "oidc-client-ts" for JavaScript, "passport-openidconnect" for Node.js, "goth" for Golang).  
...
3. Create a new application (e.g., "My$iOS/SwiftApp", type **Regular Web App**).  
4. Configure the callback URI (e.g., '$com.ssojet.app://oauth2callback' specific to $iOS/Swift, like 'http://localhost:3000/auth/callback' for Node.js).  
...
#### Step 3.2: Modify the Existing $iOS/Swift Project  

##### Substep 3.2.1: Install Dependencies  
- Specify the OIDC library installation command for $iOS/Swift (e.g., 'npm install' for Node.js, 'go get' for Golang, 'dotnet add package' for .NET).  
...
- Tailor to $iOS/Swift conventions (e.g., **Passport** for Node.js, **OpenIdConnect middleware** for .NET, **Goth** for Golang).  
...
- Use $iOS/Swift-specific templating or UI logic (e.g., **EJS** for Node.js, **Razor** for .NET, **html/template** for Golang).  
...
- Provide code snippets specific to $iOS/Swift (e.g., **Express routes** for Node.js, **Razor Page handlers** for .NET, **Mux handlers** for Golang).  
...
List considerations tailored to $iOS/Swift:  
...
- Link to relevant $iOS/Swift-specific OIDC library docs (e.g., **Passport for Node.js**, **Goth for Golang**).  

---

## Customization Instructions  
- Replace '$iOS/Swift' with the target language (e.g., **Node.js**, **ASP.NET Core**, **Golang**).  
