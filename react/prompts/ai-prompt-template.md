# AI Prompt Template for SSOJet React Integration Guide
...
List prerequisites tailored to $React, including:  
- An existing $React application with a login page.  
- Basic knowledge of $React and its common tools/frameworks (e.g., ${FRAMEWORK_TOOLS}).  
...
- Required libraries or packages specific to $React for OIDC (e.g., "oidc-client-ts" for JavaScript, "passport-openidconnect" for Node.js, "goth" for Golang).
...
3. Create a new application (e.g., "My$ReactApp", type **Regular Web App**).  
4. Configure the callback URI (e.g., '$/auth/callback' specific to $React, like 'http://localhost:3000/auth/callback' for Node.js).  
...
#### Step 3.2: Modify the Existing $React Project  

##### Substep 3.2.1: Install Dependencies  
- Specify the OIDC library installation command for $React (e.g., 'npm install' for Node.js, 'go get' for Golang, 'dotnet add package' for .NET).  
...
- Tailor to $React conventions (e.g., **Passport** for Node.js, **OpenIdConnect middleware** for .NET, **Goth** for Golang).  
...
- Use $React-specific templating or UI logic (e.g., **React components** for Next.js, **EJS** for Node.js, **Razor** for .NET, **html/template** for Golang).  
...
List considerations tailored to $React:  
...
- Link to relevant $React-specific OIDC library docs (e.g., **Passport for Node.js**, **Goth for Golang**).

---

## Customization Instructions  
- Replace '$React' with the target language (e.g., **Node.js**, **ASP.NET Core**, **Golang**).  
